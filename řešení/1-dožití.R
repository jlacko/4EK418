...
raw_hope <- czso::czso_get_table("130140") %>% 
   filter(casref_do == as.Date("2019-12-31 UTC") & # poslední řez
...
# obrázek okresů
ggplot() +
  geom_sf(data = podklad_okresy, aes(fill = hodnota)) +
  facet_grid(vars(pohlavi_txt)) +
  theme_void() +
  labs(title = "naděje dožití při narození")

# obrázek orp / jako okresy, měním jenom podkladový dataset
ggplot() +
  geom_sf(data = podklad_orp, aes(fill = hodnota)) +
  facet_grid(vars(pohlavi_txt)) +
  theme_void() +
  labs(title = "naděje dožití při narození")
