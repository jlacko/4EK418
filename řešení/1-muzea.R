...
stredocesky <- RCzechia::kraje("low") %>% 
...
ggplot() +
  geom_sf(data = stredocesky, fill = "gray85") +
  geom_sf(data = muzea_sf, color = "red", pch = 4) +
  geom_sf_text(data = muzea_sf, aes(label = mesto), nudge_y = .05) # malé nudge, aby nápis a bod nebyly přes sebe