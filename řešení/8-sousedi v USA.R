...
sousedi_kralovny <- karolina %>% 
  poly2nb()

sousedi_veze <- karolina %>% 
  poly2nb(queen = F)

rozdily <- setdiff.nb(sousedi_kralovny,
                      sousedi_veze)

# okres s největšími rozdíly
karolina$NAME[which.max(lengths(rozdily))]

# kdo je Franklin? proč se liší??
leaflet() %>% 
  addProviderTiles("CartoDB.Positron") %>% 
  addPolygons(data = subset(karolina, NAME != "Franklin"),
              label = ~ NAME) %>% 
  addPolygons(data = subset(karolina, NAME == "Franklin"),
              label = ~ NAME,
              color = "red") 

# sousedi okresu s hezkým jménem...
sousedi_mecklenburg <- unlist(sousedi_kralovny[karolina$NAME == "Mecklenburg"])

# jména sousedů
karolina$NAME[sousedi_mecklenburg]