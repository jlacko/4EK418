...
V tomto kroku bod zaměřte - dohledejte jeho souřadnice a vypište je ve formátu `{sf}` balíčku. Prosím geokódujte některou z probíraných funkcí proti externí APIně, za zadání bodu natvrdo body nejsou (a je to hloupá ztráta).
...
Okolo *vlastního* bodu vytvořte okolí (buffer) s obvodem rovným polovině vzdálenosti mezi vlastním a zadaným bodem, okolo *zadaného* bodu (VŠE) vytvořte okolí s průměrem 25 kilometrů. Oba buffery spojte do jednoho konvexního polygonu "okolí". Při konstrukci věnujte pozornost definicím obvodu a průměru.
...
Spočtěte tyto obce + vypište celkovou plochu obou typů průsečíku; očekávám tedy 4 čísla (dva součty v kusech obcí, dvě plochy v plošných mírách).
...
Zobrazte na mapě - řeky už nepotřebujeme - jako relativní údaj (hustotu vyjádřenou v kusech lidí na km²) pro oba polygony. Očekávám tedy dvě čísla (za polygon a za doplněk).
...
Odhadněte s využitím předpokladu konstantní hustoty obyvatel německé národnosti ze sčítání lidu počet Němců přítomných v roce 1930 ve vašem polygonu a ve zbytku Česka. Opět očekávám dvě čísla - za polygon a za doplněk, tentokráte jako dvě absolutní čísla (kusy Němců).

Jako kontrolu (třetí číslo :) ověřte rozdíl mezi součtem vámi spočtených čísel a hodnotou 3,149,820 (oficiální hodnota ze sčítání) a krátce komentujte. Pro předejití pochybnostem: při odečtení součtu od známého celku - což sice garantuje součet do nuly, ale informační hodnotu nepřináší - nelze uvažovat plný počet bodů.
...
Spočtěte průměrný podíl zemědělské půdy (uvažujte [raster ze služby Copernicus / družice Sentinel](https://github.com/jlacko/4EK418/blob/master/cvi%C4%8Den%C3%AD/5-zemedelska-obec.R#L7)) ve vašem polygonu a zbytkové části ČR; zakreslete do mapy jako relativní číslo (nezatížené rozdílem ploch). Očekávám tedy dvě relativní čísla (procenta). Výstup krátce diskutujte - jak se oblasti liší, co rozdíl může vysvětlit?
...
Očekávám tedy dva výstupy: teplotu ve vašem bodě jako číslo, a mapu 6400 bodů s teplotou namapovanou na škálu barvy jako obrázek.
...
ggplot() +
  ...
  theme_void()
```
## Úkol devátý: Prostorová statistika (15 b)
...
Vlastní model uvažujte lineární, i.e. `stats::lm()`. O výsledku podejte zprávu = očekávám jeden test, a sumář jednoho modelu se slovním zhodnocením.
...
## Úkol desátý: Analýza volebních výsledků (15 b)

Z ofiko volebních výsledků ze stránek [https://www.volby.cz](https://www.volby.cz) máme výsledky charizmatického politika; poznáte v jaké části ČR je těžiště jeho podpory? 
...
- uplatněte libovolný LISA algoritmus pro dohledání clusterů *významné* podpory; definici významnosti nechám na nechám na vašem vkusu, ale chci vědět jakou jste vybrali (jednostranný, oboustranný, kvantil normálního, poissonova nebo negativně binomického rozdělení...) a proč 
- zjistěte zeměpisný střed největšího spojitého významného clusteru
- zjistěte algoritmicky adresu zeměpisného středu polygonu (ze souřadnic na text); stačí obec

Očekávám tedy: jednu mapu s výsledky politika, jeden set souřadnic středu polygonu významné podpory a jedno jméno obce + krátký slovní komentář k definici významnosti.
